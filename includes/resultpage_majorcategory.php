  ...
  <table width="600">
    ...
    <?php for($jn=1; $jn<=$judge; $jn++){ ?>
		...
		$mjrCat=select("majorcategories ORDER BY  `majorcategories`.`sort` ASC");
	  	echo mysqli_num_rows(findData('major_result','judge',$jn))==($canNumMs+$canNumMr)*mysqli_num_rows($mjrCat)?"bgcolor='#06C'":"bgcolor='#F33'";
		?> scope="col">
			<a href="../?judge=<?=$jn?>&award=Major+Awards" 
	...
	for($e=1; $e<=$judge; $e++){
		for($m=1; $m<=$canNumMs; $m++){
			...
			while($mjr=mysqli_fetch_array($mjrCat)){
				$s{$e}{$m}+=score($mjr[1], $e, $m, "major_result");
			}
		}
	}
